# ============================================================
#  上证50指数多步预测 —— 双流融合架构（N+1 Token Dual-Stream LDENet）
#
#  相比原版 chapter54.py 的改动：
#    1. AttentionFusion → MultiStepAttentionFusion
#       token 序列从长度 2 扩展为长度 N+1：
#         [H_feature, H_lde_1, H_lde_2, ..., H_lde_N]
#       步长之间通过 Self-Attention 显式建模依赖关系
#    2. N 个独立 DualStreamSDENet → 单个 MultiStepSDENet
#       内含 N 份独立 LDEModule（各步长随机流独立）
#       共享一份 FeatureStream 和一份 PredictionHead
#    3. PredictionHead 输入维度 2*hidden → hidden
#       （Attention 输出不再展平，每步直接取对应 token）
#    4. forward_all / align_outputs 改为单模型接口
#       数据对齐改为输入端截尾（取各步长最短公共长度）
# ============================================================
...
#  1. 超参数
# ============================================================
parser = argparse.ArgumentParser(description='N+1 Token Dual-Stream LDENet for SSE50')
parser.add_argument('--epochs',         type=int,   default=100)
parser.add_argument('--n_steps',        type=int,   default=4,    help='预测步数，即同时预测 t+1…t+n_steps')
parser.add_argument('--lr',             type=float, default=1e-4, help='主网络学习率')
...
parser.add_argument('--dir_weight',     type=float, default=1.0,  help='方向损失权重')
parser.add_argument('--sigma_min',      type=float, default=0.1,  help='sigma 下界')
parser.add_argument('--cls_weight',     type=float, default=1.0,  help='分类交叉熵损失权重')
...
def price_to_return(x_price: np.ndarray) -> np.ndarray:
    """价格水平矩阵 [N,D] → 相邻差分收益率矩阵 [N,D-1]"""
    return np.diff(x_price, axis=1)
...
X_trains  = [to_tensor(x).unsqueeze(1) for x in x_trains]
Y_trains  = [to_tensor(y)              for y in y_trains]
C_trains  = [to_tensor(a)              for a in aux_trains]
Yp_trains = [to_tensor(p)              for p in y_prev_trains]
...
print(f'[输入维度] input_dim = D_RET = {D_RET}')
...
class DriftNet(nn.Module):
    """f(t, x)：输入输出均为 [B, hidden_dim]，当前时间无关。"""
...
class DiffusionNet(nn.Module):
    ...
    g(t, x)：输出 logit（不含 Sigmoid）。
      False → Sigmoid × sigma 作为扩散缩放
      True  → 直接送入 BCEWithLogitsLoss
    """
...
class LDEModule(nn.Module):
    ...
    x_seq [B,T,D] → 取最后时间步 → downsample → Euler-Maruyama 积分 → H_lde [B,D_h]
    training_diffusion=True → 返回 diffusion logit [B,1]
    """
...
class FeatureStream(nn.Module):
    """
    全序列 mean pooling + 5 个辅助特征 → Linear + ReLU → H_feature [B,D_h]
    """
...
# ── 9.5 N+1 token 注意力融合（核心改动）────────────────────
class MultiStepAttentionFusion(nn.Module):
    """
    N+1 token Self-Attention 融合模块

    token 序列（长度 = n_steps + 1）：
      位置 0          : H_feature  [B, D_h]  全序列统计特征（下路，全局上下文）
      位置 1 … n_steps: H_lde_k   [B, D_h]  各步长随机流（上路）

    相比原版 2-token 设计，步长 t+k 的预测可以通过 Attention 感知其他步长
    的随机流状态，显式建模多步之间的动态依赖。

    数据流：
      [H_feature, H_lde_1, ..., H_lde_N]
        → stack → seq [B, N+1, D_h]
        → MultiheadAttention (Q=K=V=seq)
        → 残差 + LayerNorm
        → 取位置 1…N → list of [B, D_h] × N
    """
    def __init__(self, hidden_dim: int, num_heads: int, n_steps: int):
        super().__init__()
    ...
    def forward(self,
                H_feature: torch.Tensor,
                H_lde_list: list) -> list:
        """
        H_feature  : [B, D_h]
        H_lde_list : list of [B, D_h]，长度 = n_steps
        返回        : list of [B, D_h]，长度 = n_steps
        """
        tokens  = [H_feature.unsqueeze(1)] + [h.unsqueeze(1) for h in H_lde_list]
        seq     = torch.cat(tokens, dim=1)          # [B, N+1, D_h]
        attn_out, _ = self.attn(seq, seq, seq)       # [B, N+1, D_h]
        attn_out = self.norm(attn_out + seq)         # 残差 + LayerNorm
        # 取位置 1…N，各步长拿到 [B, D_h]
        return [attn_out[:, k + 1, :] for k in range(self.n_steps)]


# ── 9.6 输出三头 ─────────────────────────────────────────────
class PredictionHead(nn.Module):
    """
    共享预测头：接收单步融合向量 [B, hidden_dim]，输出三路预测。

    相比原版：in_dim = hidden_dim（原为 2*hidden_dim，因原版展平了 [B,2,D]）
    三头：mean [B] / log_sigma [B] / dir_logit [B,2]
    """
    def __init__(self, hidden_dim: int, mean_scale: float = 0.3):
        super().__init__()
    ...
    def forward(self, x: torch.Tensor):
        h         = self.trunk(x)
        mean      = torch.tanh(self.head_mean(h)) * self.mean_scale
        mean      = mean.squeeze(-1)
...
#  10. 完整单模型多步预测网络（核心改动）
# ============================================================
class MultiStepSDENet(nn.Module):
    """
    单模型 N+1 token Dual-Stream Fusion SDENet

    数据流：
                     x_seq [B,T,D]  +  aux [B,5]
                    /                              \\
           [下路：FeatureStream × 1]     [上路：LDEModule × N_STEPS]
           mean pooling + aux              各步长独立采样 Lévy 噪声
           H_feature [B,D_h]              H_lde_1…N [B,D_h]
                    \\                              /
                     [MultiStepAttentionFusion]
                     seq [B, N+1, D_h]  Self-Attention
                     取位置 1…N → list of [B, D_h]
                              |
                     [PredictionHead × 1（共享权重）]
                     (mean_k, log_sigma_k, dir_logit_k) × N

    设计说明：
      - LDEModule × N：各步长随机流独立（噪声路径不同）
      - FeatureStream × 1：全序列统计特征与步长无关
      - PredictionHead × 1：共享权重，节省参数，各步长结构相同
        如需各步长独立头，将 self.pred_head 改为 nn.ModuleList 即可
    """
    def __init__(self, input_dim: int, hidden_dim: int = 64,
                 num_heads: int = 4, layer_depth: int = 25,
                 sigma: float = 0.5, n_steps: int = 4,
                 mean_scale: float = 0.3):
        super().__init__()
        self.n_steps = n_steps

        self.feature_stream = FeatureStream(input_dim, hidden_dim, n_aux=N_AUX)
        self.lde_list       = nn.ModuleList([
            LDEModule(input_dim, hidden_dim, layer_depth, sigma)
            for _ in range(n_steps)
        ])
        self.attn_fusion = MultiStepAttentionFusion(hidden_dim, num_heads, n_steps)
        self.pred_head   = PredictionHead(hidden_dim, mean_scale=mean_scale)
    ...
    def forward(self, x_seq: torch.Tensor, aux_feat: torch.Tensor,
        ...
        x_seq     : [B, T, input_dim]
        aux_feat  : [B, n_aux]
        training_diffusion=True  → list of logit [B,1]，长度 N（判别器训练）
        training_diffusion=False → list of (mean,log_sigma,dir_logit)，长度 N
        """
        if training_diffusion:
            return [lde(x_seq, training_diffusion=True) for lde in self.lde_list]

        H_feature  = self.feature_stream(x_seq, aux_feat)
        H_lde_list = [lde(x_seq, training_diffusion=False) for lde in self.lde_list]
        fused_list = self.attn_fusion(H_feature, H_lde_list)
        return [self.pred_head(f) for f in fused_list]
...
print(f'[Model] MultiStepSDENet 总参数量: {count_parameters(net):,}')

build_levy_pool(hidden_dim=HIDDEN_DIM, device=device)

# 主网络优化器（downsample + drift + feature_stream + attn_fusion + pred_head）
optimizer_F = optim.SGD(
    [{'params': net.feature_stream.parameters()},
     {'params': net.attn_fusion.parameters()},
     {'params': net.pred_head.parameters()}]
    + [{'params': lde.downsample.parameters()} for lde in net.lde_list]
    + [{'params': lde.drift.parameters()}      for lde in net.lde_list],
    lr=args.lr, momentum=0.9, weight_decay=5e-4,
)
# 扩散判别器优化器
optimizer_G = optim.SGD(
    [{'params': lde.diffusion.parameters()} for lde in net.lde_list],
...
print(f'[损失] _diff_std={_diff_std:.6f}  '
      f'dir_weight={args.dir_weight}  cls_weight={args.cls_weight}')


def dpl_loss(y_diff: torch.Tensor, mean_diff: torch.Tensor) -> torch.Tensor:
    """Hinge 方向损失：DPL = mean(max(0, margin - y_diff·mean_diff))"""
    margin  = float(_diff_std)
    product = y_diff * mean_diff
    return torch.clamp(margin - product, min=0.0).mean()


def cls_loss(y_diff: torch.Tensor, dir_logit: torch.Tensor) -> torch.Tensor:
    """方向分类 CrossEntropy：label = (y_diff > 0).long()"""
    dir_label = (y_diff > 0).long()
    return F.cross_entropy(dir_logit, dir_label)


def combined_loss(y_diff, mean_diff, log_sigma, dir_logit):
    """联合损失：MSE + λ_hinge·DPL + λ_cls·CE"""
    loss_mse   = mse_loss_fn(y_diff, mean_diff)
    loss_dpl   = dpl_loss(y_diff, mean_diff)
    loss_ce    = cls_loss(y_diff, dir_logit)
    total      = loss_mse + args.dir_weight * loss_dpl + args.cls_weight * loss_ce
    return total, loss_mse, loss_dpl, loss_ce


def nll_loss(y, mean, log_sigma):
    sigma = F.softplus(log_sigma).clamp(min=args.sigma_min) + 1e-3
    return torch.mean(torch.log(sigma) + (y - mean) ** 2 / (2.0 * sigma ** 2))


def mse_loss_fn(y, mean):
...
#  13. 前向传播公共逻辑
#
#  新版：单模型，取各步长最短公共长度后统一输入。
#  各步长 X_list[i] 行数差异仅来自 drop_tail（末尾 0~N-1 行），
#  用步长 0 的 X/C 作为公共输入是合理近似。
# ============================================================
def get_min_len(tensor_list: list) -> int:
    return min(t.shape[0] for t in tensor_list)


def forward_all(net, X_list, C_list, training_diffusion=False):
    """
    单模型前向。取各步长数据的最短公共长度后调用 net.forward 一次。

    返回：
      False → list of (mean, log_sigma, dir_logit)，长度 N_STEPS
      True  → list of logit [B,1]，长度 N_STEPS
    """
    min_len = get_min_len(X_list)
    x = X_list[0][:min_len]
    c = C_list[0][:min_len]
    return net(x, c, training_diffusion=training_diffusion)


def align_outputs(raw_outs):
    """
    提取各步长输出。新版输入已对齐，无需 TAIL_CUTS。
    返回接口与原版完全一致：means, log_sigmas, dir_logits, min_len
    """
    means      = [o[0] for o in raw_outs]
    log_sigmas = [o[1] for o in raw_outs]
    dir_logits = [o[2] for o in raw_outs]
    min_len    = means[0].shape[0]
...
def train_epoch(epoch: int):
    ...
    for _ in range(ITER):
        ...
        # 真实输入
        preds_in    = forward_all(net, X_trains, C_trains, training_diffusion=True)
        ...
        # 噪声输入
        min_len_g   = get_min_len(X_trains)
        X_noisy     = X_trains[0][:min_len_g] + 2.0 * torch.randn_like(X_trains[0][:min_len_g])
        C_noisy     = C_trains[0][:min_len_g]
        preds_out   = net(X_noisy, C_noisy, training_diffusion=True)
    ...
    with torch.no_grad():
        ...
        targets = get_targets(Y_trains, min_len)
        mse_list, da_mean_list, da_cls_list = [], [], []
        for i in range(N_STEPS):
            pred_np = means[i].cpu().numpy()
            if not np.isfinite(pred_np).all():
                raise RuntimeError(
                    f'Epoch {epoch}, step t+{i+1}: 预测值出现 NaN/Inf。'
                    f'建议降低 lr（当前={args.lr}）或减小 sigma。'
                )
            tgt_np   = targets[i].cpu().numpy()
            cls_pred = dir_logits[i].argmax(dim=-1).cpu().numpy()
            cls_true = (tgt_np > 0).astype(int)
            mse_list.append(mean_squared_error(tgt_np, pred_np))
            da_mean_list.append(np.mean(np.sign(pred_np) == np.sign(tgt_np)))
            da_cls_list.append(np.mean(cls_pred == cls_true))

    da_mean_str = '  '.join(f't+{i+1}:{da_mean_list[i]:.3f}' for i in range(N_STEPS))
    da_cls_str  = '  '.join(f't+{i+1}:{da_cls_list[i]:.3f}'  for i in range(N_STEPS))
    print(f'[Train] Epoch {epoch:3d} | '
          f'Loss:{total_loss/ITER:.4f}  MSE:{total_mse/ITER:.4f}  '
          f'DPL:{total_dpl/ITER:.4f}  CE:{total_cls/ITER:.4f}')
    print(f'         DA(mean):{da_mean_str}')
    print(f'         DA(cls) :{da_cls_str}')
...
def test_epoch(epoch: int):
    ...
    with torch.no_grad():
        for _ in range(ITER_TEST):
            raw    = forward_all(net, X_tests, C_tests, training_diffusion=False)
            means, log_sigmas, dir_logits, min_len = align_outputs(raw)
    ...
    print(f'[ Test] Epoch {epoch:3d} | RMSE:{(total_loss/ITER_TEST)**0.5:.6f} | '
          f'DA(mean):{da_mean_str} | DA(cls):{da_cls_str}')
    return tuple(mse_list)
...
with torch.no_grad():
    raw_val = forward_all(net, X_vals, C_vals, training_diffusion=False)
...
# ============================================================
with torch.no_grad():
    raw_test = forward_all(net, X_tests, C_tests, training_diffusion=False)
    final_means, final_log_sigmas, _, test_min_len = align_outputs(raw_test)
...
print("  d = np.load('ADE2_data_t+1.npz')")
