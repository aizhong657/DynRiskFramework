...
#    7. log_sigma 纳入训练
#    8. 删除了原来的高斯 nll_loss，替换为 Student-t NLL
#    9. 删除 dir_logit
# ============================================================
...
# ── 9.6 输出双头（mean + log_sigma）────────────────────────
class PredictionHead(nn.Module):
    """
    两层全连接 + 双头输出：
      • mean      [B]    连续值预测（差分均值）
      • log_sigma [B]    不确定性估计（参与 Student-t NLL 训练）

    两头均纳入训练损失，log_sigma 通过 Student-t NLL 获得梯度。
    删除原 dir_logit 分类头——其与 mean 共享 trunk，不构成独立校验。
    """
    def __init__(self, hidden_dim: int, mean_scale: float = 0.5):
        super().__init__()
        in_dim = 2 * hidden_dim
        ...
        # 均值头：tanh × mean_scale 约束输出范围
        self.head_mean      = nn.Linear(hidden_dim, 1)
        # log_sigma 头：参与 Student-t NLL 损失
    ...
    def forward(self, x: torch.Tensor):
        ...
        mean      = mean.squeeze(-1)                # [B]
        log_sigma = self.head_log_sigma(h).squeeze(-1)  # [B]
...
class DualStreamSDENet(nn.Module):
    """
    双流融合架构：Dual-Stream Fusion SDENet

    数据流图：
                         x_seq [B,T,D]
                        /              \\
               [上路：LDEModule]   [下路：FeatureStream]
               Euler-Maruyama      mean pooling + corr_dim
               + Lévy 噪声         + Linear
                        \\              /
                         [AttentionFusion]
                         MultiheadAttention
    ...
    ────
    input_dim  : 相空间嵌入维数 D
    hidden_dim : 上/下路统一隐层维度
    num_heads  : MultiheadAttention 头数（hidden_dim % num_heads == 0）
    layer_depth: Euler-Maruyama 步数
    sigma      : 扩散项缩放系数
    """
    ...
    def forward(self, x_seq: torch.Tensor, corr_dim: torch.Tensor,
        ...
        参数
        ────
        x_seq              : [B, T, input_dim]  相空间重构序列
        corr_dim           : [B, 1]             关联维数特征
        training_diffusion : True  → 仅返回 diffusion logit [B, 1]（判别器训练）
                             False → 返回 (mean [B], log_sigma [B])

        返回
        ────
        False → (mean, log_sigma)，各形状 [B]
        True  → diffusion logit [B, 1]
        """
        if training_diffusion:
            return self.lde(x_seq, training_diffusion=True)   # [B, 1]

        # 上路：随机流
        H_lde     = self.lde(x_seq, training_diffusion=False) # [B, hidden_dim]

        # 下路：特征流
        H_feature = self.feature_stream(x_seq, corr_dim)      # [B, hidden_dim]

        # 融合
        fused     = self.attn_fusion(H_lde, H_feature)        # [B, 2*hidden_dim]

        # 输出双头
        mean, log_sigma = self.pred_head(fused)    # [B], [B]
        return mean, log_sigma
...
nets = nn.ModuleList([
    DualStreamSDENet(
        input_dim=SEQ_LEN,    # 22（收益率），不再是 23（价格水平）
...
# ── 可学习 Student-t 自由度 nu（每步长独立，纳入 optimizer_F）──────
# nu > 2 保证方差存在；softplus 保证严格正，+2 保证 > 2
# log_nu 初始值使 softplus(log_nu)+2 ≈ nu_init
import math
...
print(f'[Model] Student-t 初始自由度 nu ≈ {args.nu_init:.1f}（可学习）')
...
def forward_all(nets, X_list, C_list, training_diffusion=False):
    ...
    返回
    ────
    training_diffusion=False → list of (mean [N], log_sigma [N])
    training_diffusion=True  → list of diffusion logit [N, 1]
    """
    return [
...
def align_outputs(raw_outs):
    ...
    raw_outs : list of (mean [N], log_sigma [N])
    返回
    ────
    means      : list of [min_len] Tensor
    log_sigmas : list of [min_len] Tensor
    min_len    : int
    """
    clipped_means      = []
    clipped_log_sigmas = []
    for i in range(N_STEPS):
        mean_i, ls_i = raw_outs[i]
        cut = TAIL_CUTS[i]
        if cut > 0:
            mean_i = mean_i[:-cut]
            ls_i   = ls_i[:-cut]
        clipped_means.append(mean_i)
        clipped_log_sigmas.append(ls_i)

    min_len    = min(m.shape[0] for m in clipped_means)
    means      = [m[:min_len]  for m in clipped_means]
    log_sigmas = [ls[:min_len] for ls in clipped_log_sigmas]
...
def train_epoch(epoch: int):
    ...
    for _ in range(ITER):
        ...
        means, log_sigmas, min_len = align_outputs(raw)
        targets = get_targets(Y_trains, min_len)

        # combined_loss 返回 (total, mse, dpl, nll) 四元组
        loss_parts = [
        ...
        # 噪声输入 → label=1（扰动 x_seq，保持 corr_dim 不变）
    ...
    # ── 计算训练 MSE、方向准确率（带 NaN 检测）──────────────────
    with torch.no_grad():
        raw     = forward_all(nets, X_trains, C_trains, training_diffusion=False)
        means, _, min_len = align_outputs(raw)
        targets = get_targets(Y_trains, min_len)
        mse_list     = []
        da_mean_list = []
        nu_list      = []
        for i in range(N_STEPS):
            ...
            nu_val = (F.softplus(log_nus[i]) + 2.0).item()
            nu_list.append(nu_val)

    da_str = '  '.join(f't+{i+1}:{da_mean_list[i]:.3f}' for i in range(N_STEPS))
...
with torch.no_grad():
    ...
    val_means, val_log_sigmas, val_min_len = align_outputs(raw_val)
...
with torch.no_grad():
    ...
    final_means, final_log_sigmas, test_min_len = align_outputs(raw_test)
