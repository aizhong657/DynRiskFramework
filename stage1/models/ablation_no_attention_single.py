...
parser.add_argument('--embed_dim',     type=int,   default=28,  # 嵌入维数 D，敏感性分析时在此处修改
                    help='相空间重构嵌入维数 D（敏感性分析参数）')
...
#  3. 数据加载与预处理
# ============================================================
data = pd.read_csv(DATA_DIR / "cnpc_data.csv")
...
# ============================================================
#  【敏感性分析参数】嵌入维数 D 统一在此由 args.embed_dim 控制，
#  修改时只需调整上方 argparse 的 default 值（或命令行传入 --embed_dim N）。
# ============================================================
D, TAU = args.embed_dim, 1
...
# ============================================================
for i in range(N_STEPS):
    fname = f'ADE3_{step_labels[i]}.npz'
    np.savez(fname,
