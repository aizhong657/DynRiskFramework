...
class ETGPDTransformer(nn.Module):
    ...
    def fit_gpd_correction(
        ...
        # 超额残差 ε_t = max(loss_t - VaR_t, 0)（论文式：仅保留穿透部分）
        residuals = np.maximum(train_losses - train_var, 0.0)
        pos       = residuals[residuals > 0]
        if len(pos) < 10:
            return {"xi": 0.1, "beta": float(np.std(train_losses)) * 0.1,
                    "u": 0.0, "scale_factor": 1.0, "n_excess": 0}
        u         = np.percentile(pos, 90)            # 超额残差的90%分位为阈值
        excess    = pos[pos > u] - u
    ...
    def apply_gpd_correction(
        ...
        校正公式（与论文一致）：
          VaR_final = VaR_base × s_f
          ES_α      = (VaR_α + β) / (1 - ξ)   （去阈值化解析估计量）
        """
        sf  = gpd_params.get("scale_factor", 1.0)
        xi  = gpd_params.get("xi", 0.1)
        beta = gpd_params.get("beta", 0.01)
        var_corr = var_pred * sf
        # ES 用论文式的去阈值化 GPD 解析公式：ES = (VaR + β)/(1 - ξ)
        denom    = max(1.0 - xi, 0.1)
        es_corr  = (var_corr + beta) / denom
